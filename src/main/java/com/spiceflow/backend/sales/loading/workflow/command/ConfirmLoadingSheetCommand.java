...
public record ConfirmLoadingSheetCommand(@Nullable String comment) implements WorkflowCommand<LoadingSheet, LoadingSheetState> {

    public ConfirmLoadingSheetCommand() {
        this(null);
    }

    @Override
    public String getCommandName() {
        return "ConfirmLoadingSheet";
    }

    @Override
    public LoadingSheetState getTargetState() {
        return LoadingSheetState.CONFIRMED;
    }
    ...
    public void validate(LoadingSheet aggregate, WorkflowContext context) {
        ...
        if (aggregate.getItems().isEmpty()) {
            throw new BusinessRuleViolationException("Cannot confirm Loading Sheet without any items");
        }
    }
}
