...
public record ApproveRepOrderCommand(@Nullable String comment) implements WorkflowCommand<RepOrder, RepOrderState> {

    public ApproveRepOrderCommand() {
        this(null);
    }

    @Override
    public String getCommandName() {
        return "ApproveRepOrder";
    }

    @Override
    public RepOrderState getTargetState() {
        return RepOrderState.APPROVED;
    }
    ...
    public void validate(RepOrder aggregate, WorkflowContext context) {
        ...
        if (aggregate.getShops().isEmpty()) {
            throw new BusinessRuleViolationException("Cannot approve Rep Order without any shop orders");
        }
    }
}
