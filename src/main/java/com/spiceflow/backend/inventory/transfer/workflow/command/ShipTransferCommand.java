...
public record ShipTransferCommand(@Nullable String comment) implements WorkflowCommand<WarehouseTransfer, WarehouseTransferState> {

    public ShipTransferCommand() {
        this(null);
    }
    ...
    public WarehouseTransferState getTargetState() {
        return WarehouseTransferState.SHIPPED;
    }
    ...
    public void validate(WarehouseTransfer aggregate, WorkflowContext context) {
        ...
        for (WarehouseTransferLine line : aggregate.getLines()) {
            if (line.getShippedQty().compareTo(BigDecimal.ZERO) <= 0) {
                throw new BusinessRuleViolationException("Shipped quantity must be positive for product " + line.getProductId());
            }
        }
    }
}
